...
def main():
    ...
    parser.add_argument("-i", "--ignore", nargs='+', default=["head_det",
                        "headdet", "head det"], help="annotator(s) to ignore")
    parser.add_argument("-a", "--annotator", action="store_true",
                        help="output annotator names")
    parser.add_argument("-d", "--date_limit",
                        help="upper bound for annotation date")
    parser.add_argument("-m", "--merge", action="store_true",
                        help="try to clean up annotations by merging them and "
                        + "dropping unmerged ones")
    args = parser.parse_args()

    name = args.path.replace(os.sep, "")
    dateLimit = None if args.date_limit is None else datetime.strptime(
        args.date_limit, DATE_FORMAT)

    args.annotator = args.annotator and not args.merge

    if args.annotator:
        csvfileAnnotator = open(name + "_annotator.csv", 'w', newline='')
        csvwriterAnnotator = csv.writer(
            csvfileAnnotator, delimiter="\t", lineterminator="\n")
        ...
        for (dir, dirs, filenames) in os.walk(args.path):
            for file in filenames:
                if ".json" in file and ".log.json" not in file:
                    ...
                    imageName = path.splitext(
                        path.join(path.abspath(dir), file))[0]
                    ...
                    if args.merge:
                        ...
                        if not trash:
                            ...
                            if len(annotations) > 1:
                                ...
                                for annotation2 in annotations[1:]:
                                    ...
                                    annotation1 = mergeAnnotations(
                                        annotation1, annotation2)
                            # print(np.array(
                            #     annotation1, dtype=np.uint16)[:, 4].sum())
                        ...
                        for i, datum in enumerate(data):
                            ...
                            if (
                                ...
                                (maxDate is None or maxDate < date)
                                # and datum["annotator"] == "none"
                            ):
                                ignore = False
                                for name in args.ignore:
                                    if name in datum["annotator"]:
                    ...
                    if write:
                        ...
                        if args.count > 1:
                            ...
                            fileName, fileExt = path.splitext(
                                path.basename(imageName))
                            index = fileName.rfind('n')
                            frame = int(fileName[index+1:])
                            fileName = fileName[:index+1]
                            for n in range(-args.count+1, annotation1):
                                editedImageName = path.join(
                                    dirName, fileName+str(frame+n)+fileExt)
                        ...
                        if args.annotator:
                            csvwriterAnnotator.writerow(
                                imageNames+[annotator]+heads)
