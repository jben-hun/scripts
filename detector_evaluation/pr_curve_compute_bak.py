...
parser.add_argument("-m", "--bb_match_metric", type=str,
                    choices=["IOU", "IOA", "IOD", "SPIA"], default="IOU")
...
parser.add_argument("--ignore_match_metric", type=str,
                    choices=["IOU", "IOA", "IOD", "SPIA"], default="IOD")
...
def readSkeletonJSONFile(jsonPath):
    ...
    for skeletonFile in skeletonDataset:
        ...
        for skel in skeletonFile["skeletons"]:
            ...
            bb = [min(x), min(y), max(x), max(y), sum(scores)/len(scores),
                  list(zip(x, y))]
...
def getGraphValues(activeAnnsDict, ignoreAnnsDict, predsDict):
    ...
    for i in range(len(predScores)):
        ...
        newImgStats = getImgMetrics(predsDict[imgId], activeAnnsDict[imgId],
                                    ignoreAnnsDict[imgId], scoreTresh)
...
scores, precisions, recalls = getGraphValues(activeAnnsDict, ignoreAnnsDict,
                                             predictionsDict)
